...
- As a member of a \link #lttng_handle recording session handle\endlink.
  ...
  Some functions which require both a \ref api_session "recording session"
...
fields and using it, for example:

@code
struct lttng_domain domain_summary;

memset(&domain_summary, 0, sizeof(domain_summary));
@endcode
*/
struct lttng_domain {
	/// Tracing domain type.
